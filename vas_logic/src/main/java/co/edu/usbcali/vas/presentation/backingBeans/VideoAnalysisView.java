...
public class VideoAnalysisView implements Serializable {
 ...
 // DATE SELECTION
    ...
    public String action_analyze_alg_anomalous() throws Exception {
    	log.info("VideoAnalysisView action_analyze_alg_anomalous");
    	...
    	try {
    		...
    		//VALIDATE STATUS
    		//status = businessDelegatorView.isAlgAnlAvailable();
    		//VALIDATE VAS_MONITOR STATU
    		status = businessDelegatorView.monitorControllerStatus();
    		...
    		if(this.videoUrlPlayer  != null) {
    			...
    			/*String initTimeParam = String.valueOf(txtInitTime.getValue());
    			String finalTimeParam = String.valueOf(txtFinalTime.getValue());
    			
    			log.info("txtInitTime: "+initTimeParam);
    			log.info("txtFinalTime: "+finalTimeParam);*/
    			//log.info("info: " + txtAnomalousInfo.getValue().toString());
				
    			//ANALYZE WITH ALG
    			String finalVideo = businessDelegatorView.analyze_anomalousAlgFromTempLocation(
    					tempVideoFile, this.videoSize.toString(), initTime.toString().trim(), finalTime.toString().trim(), txtAnomalousInfo.getValue().toString());

    		    FacesUtils.addInfoMessage("Análisis con Eventos Anomalos finalizado exitosamente");
    		    //GET FINAL VIDEO URL 
