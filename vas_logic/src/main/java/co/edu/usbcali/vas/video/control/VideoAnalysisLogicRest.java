...
public class VideoAnalysisLogicRest implements IVideoAnalysisLogicRest {
    ...
    public String saveVideoToTempLocation(String contentType, String fileName, Long size, InputStream inputFile) throws Exception {
		...
		try {
			...
			File targetFile = new File(this.getTemp_video_folder() + fileName);
			...
			entity.setUrl(this.getWeb_server_temp() + fileName);
	...
	@Override
    @Transactional(readOnly = false, propagation = Propagation.REQUIRED)
    public String analyze_anomalousAlgFromTempLocation(String videoFileTemp, String videoSize, String initTimeParam, String finalTimeParam) throws Exception {
        log.info("1. VideoAnalysisLogic analyze ANOMALOUS AlgFromTempLocation---------------------------------");
		...
		try {
			...
			log.info("finalTimeParam: " + finalTimeParam);
				...
				//SET DATA
				AlgorithmDTO algorithmDTO = this.setAlgRestData(videoFileTemp, videoFile, this.getLocalTemp_video_folder(), this.getOutput_alg_anl_video_folder(),
						this.getExternal_program_alg_anl_location(), initTimeParam, finalTimeParam, "", "", "", "");
				
				//PROCESS VIDEO ANALYTICS ALG BY REST
				//Check controller Status
				Boolean controllerStatus = this.monitorLogicRest.monitorControllerStatus();
				log.info("VAS Controller Status: "+controllerStatus);
				
				if(controllerStatus.booleanValue() == true) {
					
					//CALL REST ANL
					this.monitorLogicRest.processVideoWithAnomalousAlgRest(algorithmDTO);
					
					//CALL ANL FILE STATUS

					if(status.equals("100")) {
						log.info("VAS processVideoWithAnomalousAlgRest STATUS: "+status);
						//FacesUtils.addWarningMessage("Analisis Finalizado correctamente");
					}else {
						log.info("VAS processVideoWithAnomalousAlgRest ERROR");
					}
					if(status.equals("VIDEO_NOT_FOUND")) {
						log.info("VAS processVideoWithAnomalousAlgRest STATUS: "+status);
						throw new RuntimeException("Error Interno: No se puede localizar video en la ruta de entrada");
					}else {
						log.info("VAS processVideoWithAnomalousAlgRest ERROR");
					}
					...
					Long finalTransferTime = System.currentTimeMillis() - initTransferTime;
					//Trace log
					systemVideoLogLogic.save_systemVideoLog("ANOMALOUS ALG - FINISH PROCESS", "VIDEO_ID: " + videoFile + " TRANSFER_TIME: " + finalTransferTime.toString());
					...
					log.error("Conexión Servicio Monitor fuera de linea");
					throw new RuntimeException("Error Interno: Conexión Servicio Monitor fuera de linea");
    ...
    public String analyze_trackerAlgFromTempLocation(String videoFileTemp, String videoSize, 
    		String posXParam, String posYParam, String posX2Param, String posY2Param) throws Exception {
