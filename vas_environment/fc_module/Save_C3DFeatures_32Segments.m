...
path_c3d_features='../c3d_module/output/video_input'
path_c3d_features_txt='./C3D_Features_txt';

 
if ~exist(path_c3d_features_txt,'dir')
     mkdir(path_c3d_features_txt)
end
 

All_Folder=dir(path_c3d_features);
All_Folder=All_Folder(3:end);
subcript='_C.txt';

for ifolder=1:length(All_Folder)
     
    Folder_Path=[path_c3d_features,'/',All_Folder(ifolder).name];
    ...
    Feature_vect=zeros(length(AllFiles),4096);
     for ifile=1:length(AllFiles)
      ...
      % Training_AnomalyDetector_public ( You can directly use .mat format if you want).
       ...
       fid1=fopen([path_c3d_features_txt,'/',All_Folder(ifolder).name,subcript],'w'); 
