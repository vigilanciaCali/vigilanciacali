...
/**
 * Extracts the frame sequence of a video file - If is URL.
 */
function extractFramesFromVideo2(config, file, progress) {
    let resolve = null;
    let db = null;
    let video = document.createElement('video');
    let canvas = document.createElement('canvas');
    ...
    return new Promise((_resolve, _) => {
        ...
        db = new PouchDB(dbName).destroy().then(() => {
            ...
            video.loop = false;
            video.playbackRate = config.playbackRate;
            var blobFile = new File([0], file, {
                type: "video/mp4",
            });
            video.src = URL.createObjectURL(blobFile);
            compatibility.requestAnimationFrame(onBrowserAnimationFrame);
            video.play();
        });
    });
