...
public class VideoAnalysisLogic implements IVideoAnalysisLogic {
	...
	public String saveVideoToTempLocation(String contentType, String fileName, Long size, InputStream inputFile,
		...
		try {
			...
			// 2. SAVE VIDEO TO SFTP TEMP
			// LOCATION--------------------------------------------------
			// validate sftp connection
			Boolean sftpAvilable = sftpLogic.isSftpAvailable();

			if (sftpAvilable.booleanValue() == true) {
				...
				// Trace log
				systemVideoLogLogic.save_systemVideoLog("TEMP VIDEO - FINISH FILE COPY",
						"VIDEO_ID: " + fileName + " TRANSFER_TIME: " + finalTransferTime.toString());
				...
				VideoType videoType = videoTypeLogic.getVideoTypeByCode(type);
				entity.setVideoType(videoType);

				VideoStatus videoStatus = videoStatusLogic.getVideoStatusByCode(Constantes.NEW);
				...
				throw new RuntimeException("El servidor remoto no se encuentra disponible en este momento, por favor intente mas tarde");
			}

		} catch (Exception e) {
			FacesUtils.addErrorMessage("Error cargando video temporal");
	...
	public String analyze_anomalousAlgFromTempLocation(String videoFileTemp, String videoSize, String initTimeParam,
		...
		try {
			...
			this.monitorLogic.processVideoWithAnomalousEventsAlg(videoFileTemp, videoFile, this.getTemp_video_folder(),
					this.getOutput_alg_anl_video_folder(), this.getExternal_program_alg_anl_location(), initTimeParam,
					finalTimeParam);
			...
			systemVideoLogLogic.save_systemVideoLog("ANOMALOUS ALG - FINISH PROCESS",
					"VIDEO_ID: " + videoFile + " PROCESSING_TIME: " + finalProcessingTime.toString());
	...
	public AlgorithmDTO setAlgRestData(String videoFileTemp, String videoFile, String tempVideoFolder,
	...
	public String analyze_trackerAlgFromTempLocation(String videoFileTemp, String videoSize, String posXParam,
		...
		try {
			...
			this.monitorLogic.processVideoWithTrackerAlg(videoFileTemp, videoFile, this.getTemp_video_folder(),
					this.getOutput_alg_trc_video_folder(), this.getExternal_program_alg_trc_location(), posXParam,
					posYParam, posX2Param, posY2Param);

			Long finalTransferTime = System.currentTimeMillis() - initTransferTime;
			// Trace log
			systemVideoLogLogic.save_systemVideoLog("TRACKER ALG - FINISH PROCESS",
					"VIDEO_ID: " + videoFile + " PROCESSING_TIME: " + finalTransferTime.toString());
	...
	@Override
	@Transactional(readOnly = false, propagation = Propagation.REQUIRED)
	public void analyze_anomalousAlg(String contentType, String fileName, Long size, InputStream inputFile)
			throws Exception {
		...
		try {
			...
			Long finalTransferTime = System.currentTimeMillis() - initTransferTime;
			// Trace log
			systemVideoLogLogic.save_systemVideoLog("ANOMALOUS ALG - FINISH FILE COPY",
			 ...
			 * //String firstRemoteFile = "Seguimiento.mp4"; InputStream inputFileTmp = new
			 * FileInputStream(localFileTmp);
			 */
			...
			// this.monitorLogic.processVideoWithAnomalousEventsAlg(videoFile,
			// this.input_video_folder, this.getOutput_video_folder(),
			// this.getAnomalous_external_program_location());
	...
	@Override
	@Transactional(readOnly = false, propagation = Propagation.REQUIRED)
	public void analyze_trackerAlg(String contentType, String fileName, Long size, InputStream inputFile)
			throws Exception {
		...
		try {
			...
			Long finalTransferTime = System.currentTimeMillis() - initTransferTime;
			// Trace log
			systemVideoLogLogic.save_systemVideoLog("FINISH FILE COPY",
					"VIDEO_ID: " + videoFile + " TRANSFER_TIME: " + finalTransferTime.toString());
			...
			// this.monitor_searchuploadedVideoRestService(videoFile);
			// this.monitorLogic.processVideoWithTrackerAlg(videoFile,
			// this.input_video_folder, this.getOutput_video_folder());
