...
public class MonitorLogicRest implements IMonitorLogicRest {
    ...
    @Override
    @Transactional(readOnly = false)
    public String controllerStatus() throws Exception {
        log.info("VAS MonitorLogicRest controllerStatus ");
        
        String status = "";
        try {

				String url = this.getServiceController() + "test/keepalive/";
				log.info("url: "+url);
				RestTemplate restTemplate = new RestTemplate();
				status = restTemplate.postForObject(url, "", String.class);
				
				log.info("VAS controller STATUS: "+status);
