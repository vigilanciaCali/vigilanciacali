function Test_ROC_AOS_MVC_NSS_time(Distortion,v,NSS,Cs,Display)
...
% This function integrate temporal features calculating the NSS from
% images generated by subtracting each frame from the previous
...
if ~strcmp(Distortion,'pristine')
    if NSS
        load(strcat('./Subtracted/AOS_NSS_Sub_Video_',num2str(v),'_pristine_Cs',num2str(Cs)));
    else
        load(strcat('./Subtracted/AOS_HOG_Sub_Video_',num2str(v),'_pristine'));
    end
    % AOS_pristine = AOS;
...
%     frames = frames(:,:,:,1:100);
    ...
    % Start in the second frame
    I = double(rgb2gray(uint8(frames(:,:,:,2))));
    % Generate image subtracting second frame from the first
    I_sub = imsubtract(I, double(rgb2gray(uint8(frames(:,:,:,1)))));
    ...
    if Display
        figure; imshow(uint8(drawPatches(frames(:,:,:,2),objbbox(1,:),1)));
    end
...
%                 nssS = nssFeat(I_sub,[objP;bgP]) / Cs;
        features_subS   = hogNSSFeat(I_sub,[objP;bgP],NSS,Cs);
        nssS = features_subS(:,end-35:end);
    end
    ...
    % Loops from the third frame to the end
    for i = 3 : size(frames,4)
        i
        ...
        % Generate image subtracting current frame from the previous
        I_sub = imsubtract(I, double(rgb2gray(uint8(frames(:,:,:,i-1)))));
        ...
        if sum(isnan(objbbox(i-1,:)))==0
            ...
            if NSS
                        % Prueba funciones antiguas
%                         nssS_nxt = nssFeat(I_sub,bbox) / Cs;
                features_sub_nxt   = hogNSSFeat(I_sub,bbox,NSS,Cs);
                nssS_nxt = features_sub_nxt(:,end-35:end);
        ...
        % the whole frame (wP)
            ...
            if NSS
                        % Prueba funciones antiguas                
%                         nssS_nxt = nssFeat(I_sub,wP) / Cs;                
                features_sub_nxt = hogNSSFeat(I_sub,wP,NSS,Cs);                
                nssS_nxt = features_sub_nxt(:,end-35:end);
                features_nxt = [hogS_nxt, nssS_nxt];
            else
                features_nxt = hogS_nxt;
        ...
        if ((sum(isnan(objP_nxt))==0) & (isempty(objP_nxt)==0))
            ...
            if flag(i) == 0
                    ...
                    if NSS
                        [hogS,~,~] = bgUpdateNSS(I,'mvc',0.05,objbbox(i,:),bgP,bgKeys,hogS,nssS,Cs);
                        [~,~,nssS] = bgUpdateNSS(I_sub,'mvc',0.05,objbbox(i,:),bgP,bgKeys,hogS,nssS,Cs);
                    else
                    ...
                    if min(pdist2(objCode,binCode,'hamming')) <= bgTsh4 
                        ...
                        if NSS
                                    %   Prueba funciones antiguas
%                                     nssSobj   = nssFeat(I_sub,objbbox(i,:)) / Cs;                                
                            features_subSobj = hogNSSFeat(I_sub,objbbox(i,:),NSS,Cs);
                            nssSobj= features_subSobj(:,end-35:end);
                        end
...
if NSS
    saveas(gcf,strcat('./Subtracted/Video_',num2str(v),'_NSS_Sub_SP_',Distortion,'_Cs_',num2str(Cs)),'epsc');
else
    saveas(gcf,strcat('./Subtracted/Video_',num2str(v),'_HOG_Sub_SP_',Distortion,'_Cs_',num2str(Cs)),'epsc');
end
%close;
...
if NSS
    if strcmp(Distortion,'S & P')
        save(strcat('./Subtracted/AOS_NSS_Sub_Video_',num2str(v),'_S_P_Cs',num2str(Cs)),'predict_accuracy','AOS','ROC_accuracy','false_p','fs','total_object_found','flag_lvl');
    else
        save(strcat('./Subtracted/AOS_NSS_Sub_Video_',num2str(v),'_',Distortion,'_Cs',num2str(Cs)),'predict_accuracy','AOS','ROC_accuracy','false_p','fs','total_object_found','flag_lvl');
    end
else
    if strcmp(Distortion,'S & P')
        save(strcat('./Subtracted/AOS_HOG_Sub_Video_',num2str(v),'_S_P'),'predict_accuracy','AOS','ROC_accuracy','false_p','fs','total_object_found','flag_lvl');
    else
        save(strcat('./Subtracted/AOS_HOG_Sub_Video_',num2str(v),'_',Distortion),'predict_accuracy','AOS','ROC_accuracy','false_p','fs','total_object_found','flag_lvl');
    end
end
