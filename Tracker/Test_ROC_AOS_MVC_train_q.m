function Test_ROC_AOS_MVC_train_q(Distortion,v,Display,varargin)
...
%     v = videos_array(vid_index);
    ...
    if ~strcmp(Distortion,'pristine')
        load(strcat('./Results/MVC_Results_Video_',num2str(v),'_pristine'),'ROC_accuracy','location_precision');
        % AOS_pristine = AOS;
    ...
    final_objbbox_frame = cell(numOfFrames, length(Q));
...
%         gtP(1,:) = [gt_center(1)-round(patch_width/2), gt_center(2)-round(patch_height/2), patch_width, patch_height];
        ...
        for i = 2 : size(frames,4)
            ...
            if ((sum(isnan(objP_nxt))==0) & (isempty(objP_nxt)==0))
                ...
                final_objbbox_frame{i,o} = objP_nxt;
            else
...
%      close;
    ...
    if strcmp(Distortion,'S & P')
         save(strcat('./Results/MVC_Results_Video_',num2str(v),'_S_P'),'predict_accuracy','AOS','ROC_accuracy','false_p','fs','total_object_found','location_precision','flag_lvl','final_objbbox','final_objbbox_frame','bgP_reg_acc');
    else
         save(strcat('./Results/MVC_Results_Video_',num2str(v),'_',Distortion),'predict_accuracy','AOS','ROC_accuracy','false_p','fs','total_object_found','location_precision','flag_lvl','final_objbbox','final_objbbox_frame','bgP_reg_acc');
    end
