function [positions, fps] = mydsst(params)

%%% TRACKER
path(path,'../AppDrivenTracker-master/functions')
path(path,'../AppDrivenTracker-master/MeanShift_Code')
path(path,'../AppDrivenTracker-master/bbox_configs')
path(path,'../AppDrivenTracker-master/matlab')
...
for frame = 1:num_frames,
    ...
    if frame == 1
        ...
        hf_den = new_hf_den; % POSITION
        hf_num = new_hf_num; % POSITION
        
        sf_den = new_sf_den;
        sf_num = new_sf_num;
    else
        % subsequent frames, update the model
        hf_den = (1 - learning_rate) * hf_den + learning_rate * new_hf_den; % POSITION
        hf_num = (1 - learning_rate) * hf_num + learning_rate * new_hf_num; % POSITION
    ...
    %save position
    positions(frame,:) = [pos target_sz]; % POSITION
